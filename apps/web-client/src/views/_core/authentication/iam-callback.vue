...
const authStore = useAuthStore(); const error = shallowRef('');
onMounted(async () => { try { await authStore.completeIamLogin(); } catch (reason) { error.value = reason instanceof Error ? reason.message : 'IAM 登录失败'; } });
</script>

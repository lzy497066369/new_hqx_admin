...
export async function getAllMenusApi() {
  if (isIamAuthEnabled()) {
    ...
    return normalizeIamMenuPaths(menus as RouteRecordStringComponent[]);
  }
  return requestClient.get<RouteRecordStringComponent[]>('/menu/all');
}

function normalizeIamMenuPaths(
  menus: RouteRecordStringComponent[],
): RouteRecordStringComponent[] {
  return menus.map((menu) => ({
    ...menu,
    children: menu.children
      ? normalizeIamMenuPaths(menu.children)
      : undefined,
    path: menu.path === '/home' ? '/dashboard/workspace' : menu.path,
    redirect: menu.redirect === '/home' ? '/dashboard/workspace' : menu.redirect,
  }));
}

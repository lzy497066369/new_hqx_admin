...
const menus = computed(() => [
  {
    handler: () => {
      router.push({ name: 'ClientEnterpriseUser' });
    },
    icon: 'lucide:user',
...
function handleNoticeClear() {
  collaborationCenter.clearNotifications();
}

function markRead(id: number | string) {
  collaborationCenter.markNotificationRead(id);
}
...
function handleMakeAll() {
  collaborationCenter.markAllNotificationsRead();
}
...
const handleClick = (item: NotificationItem) => {
  if (item.id) {
    collaborationCenter.markNotificationRead(item.id);
  }
  // 如果通知项有链接，点击时跳转
  if (item.link) {
...
const viewAllSync = () => {
  router.push('/client/company/materials').catch(() => {});
};

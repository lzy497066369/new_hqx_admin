...
export function useFormSchema(options: { fixedEnterpriseId?: MaybeRef<string | undefined> } = {}): VbenFormSchema[] {
  return [
    ...
    { component: 'Input', fieldName: 'username', label: '账号', rules: 'required' },
    {
      ...
      componentProps: { placeholder: '编辑时留空则不修改密码' },
      fieldName: 'password',
      label: '密码',
    },
...
export function useDescriptionItems(row?: EnterpriseUserItem): DescriptionsItemType[] {
  ...
  return [
    ...
    { label: '默认账号', content: row?.isDefault === 1 ? '是' : '否' },
    {
      label: '状态',
      content: () =>
        h(
...
export function useColumns(
  onStatusChange?: (
    newStatus: number,
    row: EnterpriseUserItem,
  ) => PromiseLike<boolean | undefined>,
): VxeTableGridColumns<EnterpriseUserItem> {
  return [
    ...
    { field: 'phone', title: '手机号', width: 160 },
    {
      ...
      title: '默认账号',
      width: 100,
      slots: { default: ({ row }) => (row.isDefault === 1 ? '是' : '否') },
    },
    {
      cellRender: {

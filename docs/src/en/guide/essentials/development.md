...
```json
{
  "scripts": {
    ...
    // Build the web-admin application separately
    "build:web-admin": "pnpm run build --filter=@vben/web-admin",
    ...
    // Start the web-admin application
    "dev:web-admin": "pnpm -F @vben/web-admin run dev",
...
To run the `web-admin` application:
